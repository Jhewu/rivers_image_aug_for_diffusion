...
def DataBalancer(label):
   ...
   # Write a code that will "sort" the site_path dictionary
   new_site_path = {}
   for item in sorted_list:
       new_site_path[item[0]] = site_path[item[0]][:]


   site_path = new_site_path


   for site in site_path:
       # randomly shuffle the list contents
       # so that the balancing is not deterministic
       #print(f"\nThis is the current site id {site}\n")
       ...
       while counter < image_count_aug:            # --> continue to augment in case rotation or flip rotation is not enough
           ...
           print(f"\n This is the length of the site_ids {site_ids[site]}")
           print(f"This is theta {THETA}")
           for site_img in range(site_ids[site]):
               new_destination = os.path.join(destination, f"{filename_dict[site][site_img]}_augmented_{site_img}{counter}.JPG")
               #print(f"this is {switch}")
               if counter > image_count_aug:       # --> if we reached enough images within the loop
                   ...
                   counter+=1
                   print(f"this is counter in case 1 {counter}")
                   ...
                   cv.imwrite(new_destination, augmented_img)
                   counter+=1
                   print(f"this is counter in case 2 {counter}")
                   #print(f"Augmented {site_path[site][site_img]} to {new_destination}")
           switch = switch*-1
           if switch_ended % 2 == 0:
               THETA+=5
               FACT+=0.3 # --> these THETA and FACT values ensure no black padding on final image
               #print(f"\n These are theta {THETA}, and fact {FACT}")
           switch_ended+=1
       site_number+=1
   ...
   print(f"this is max {check_vale}")
